...
@export var first_person_player_scene: PackedScene
...
func _ready():
	...
	FirstPersonNetwork.connect("player_connected", Callable(self, "_on_player_connected"))
...
func _on_host_pressed(nickname: String, skin: String):
	main_menu.hide_menu()
	FirstPersonNetwork.start_host(nickname, skin)

func _on_join_pressed(nickname: String, skin: String, address: String):
	main_menu.hide_menu()
	FirstPersonNetwork.join_game(nickname, skin, address)
...
func _add_player(id: int, player_info : Dictionary):
	...
	var player = first_person_player_scene.instantiate()
	player.name = str(id)
	player.position = get_spawn_point()
	player.set("players_container", players_container)

	players_container.add_child(player, true)
	players_container.register_player(id, player)

	var nick = FirstPersonNetwork.players[id]["nick"]
...
func get_spawn_point() -> Vector3:
	...
	var spawn_point = players_container.position
	return spawn_point

func _remove_player(id):
	if not multiplayer.is_server() or not players_container.has_node(str(id)):
		return
	players_container.unregister_player(id)
...
func _on_chat_message_sent(message_text: String) -> void:
	...
	var nick = FirstPersonNetwork.players[multiplayer.get_unique_id()]["nick"]
...
func _get_local_player() -> FirstPersonCharacter:
	var local_player_id = multiplayer.get_unique_id()
	if players_container.has_node(str(local_player_id)):
		return players_container.get_node(str(local_player_id)) as FirstPersonCharacter
	return null
